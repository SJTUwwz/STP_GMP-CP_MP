...
def MonomialConstraintx(n,r,d,fw,m_deg0,m_deg1):
    command = ""
    command += "rem0 : BITVECTOR(" + str(n) + ");\n" 
    monstr = "{:b}".format(m_deg0).zfill(n)
    command += "ASSERT rem0 = BVMOD( {}, x0_0, 0bin{});\n".format(n, monstr)
    command += "ASSERT rem0 = 0bin{};\n".format("0".zfill(n))
    command += "rem1 : BITVECTOR(" + str(n) + ");\n" 
    monstr = "{:b}".format(m_deg1).zfill(n)
    command += "ASSERT rem1 = BVMOD( {}, x1_0, 0bin{});\n".format(n, monstr)
    command += "ASSERT rem1 = 0bin{};\n".format("0".zfill(n))
    fw.write(command)

def NonzeroConstraint(fw,n):
    command = ""
    command += "ASSERT BVGT( x0_0, 0bin" + "0".zfill(n) + ");\n"
    command += "ASSERT BVGT( x1_0, 0bin" + "0".zfill(n) + ");\n"
    fw.write(command)

def NonzeroConstraintxi(fw,n, xi):
    command = ""
    command += "ASSERT BVGT( x{}_0, 0bin".format(xi) + "0".zfill(n) + ");\n"
    fw.write(command)


def MonomialConstraintxi(n,r,d,fw,m_deg,xi):
    command = ""
    command += "rem : BITVECTOR(" + str(n) + ");\n" 
    monstr = "{:b}".format(m_deg).zfill(n)
    command += "ASSERT rem = BVMOD( {}, x{}_0, 0bin{});\n".format(n, xi, monstr)
