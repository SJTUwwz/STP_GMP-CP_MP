...
def rangeVariable(n,r,fw):
    ...
    command += "\n"
    command += "ASSERT BVGT( x" + str(0) + ", 0bin" + "0".zfill(n) + ");\n"
...
def MonomialConstraint(n,r,d,fw,m_deg):
    command = ""
    command += "rem : BITVECTOR(" + str(n) + ");\n" 
    monstr = "{:b}".format(m_deg).zfill(n)
    command += "ASSERT rem = BVMOD( {}, x0, 0bin{});\n".format(n, monstr)
    command += "ASSERT rem = 0bin{};\n".format("0".zfill(n))
    fw.write(command)


def CertainMonomialConstraint(n,r,d,fw,m_deg):
    command = ""
    monstr = "{:b}".format(m_deg).zfill(n)
    command += "ASSERT x0 = 0bin{};\n".format(monstr)
    fw.write(command)